...
class SampleLossWrapper(DistributionLoss, LossFunctionWrapper):
    """
    Wraps a scoringrules ensemble-based estimation of a score function into a keras loss function,
...
@register_keras_serializable(package="mlpp_lib.losses")
class TWCRPSEnsemble(SampleLossWrapper):
    def __init__(self, num_samples, a=float("-inf"), b=float("inf")):
        super().__init__(fn=sr.twcrps_ensemble, num_samples=num_samples, a=a, b=b)
        self.num_samples = num_samples
        self.a = a
        self.b = b
    ...
    def get_config(self):
        ...
        # remove the non-serializable fn
        config.pop("fn", None)
        config.update({"num_samples": self.num_samples, "a": self.a, "b": self.b})
        return config

    @classmethod
    def from_config(cls, config):
        return cls(num_samples=config["num_samples"], a=config["a"], b=config["b"])


@register_keras_serializable(package="mlpp_lib.losses")
class WeightedCRPSEnergy(TWCRPSEnsemble):
    """Backward-compatible alias for TWCRPSEnsemble."""

    def __init__(self, num_samples, threshold=float("-inf")):
        # old class only had threshold (lower bound). upper bound stays +inf.
        super().__init__(num_samples=num_samples, a=threshold, b=float("inf"))

    def get_config(self):
        return {"num_samples": self.num_samples, "threshold": self.a}

    @classmethod
    def from_config(cls, config):
        if "threshold" in config:
            return cls(num_samples=config["num_samples"], threshold=config["threshold"])
        # Fallback if a newer config shape is encountered
        return cls(
            num_samples=config["num_samples"], threshold=config.get("a", float("-inf"))
        )
