...
class TruncatedNormalDistribution(Distribution):
    ...
    def mean(self) -> torch.Tensor:
        ...
        sn = torch.distributions.Normal(
            torch.zeros_like(self.mu_bar), torch.ones_like(self.mu_bar)
        )

        scale = (torch.exp(sn.log_prob(beta)) - torch.exp(sn.log_prob(alpha))) / (
            sn.cdf(beta) - sn.cdf(alpha)
        )
    ...
    def variance(self) -> torch.Tensor:
        ...
        return self.sigma_bar**2 * (
            1.0
            - (beta * pdf_b - alpha * pdf_a) / (CDF_b - CDF_a)
            - ((pdf_b - pdf_a) / (CDF_b - CDF_a)) ** 2
        )
    ...
    def moment(self, k):
        ...
        scale = (
            self.b ** (k - 1) * torch.exp(sn.log_prob(beta))
            - self.a ** (k - 1) * torch.exp(sn.log_prob(alpha))
        ) / (sn.cdf(beta) - sn.cdf(alpha))

        return (
            (k - 1) * self.sigma_bar**2 * self.moment(k - 2)
            + self.mu_bar * self.moment(k - 1)
            - self.sigma_bar * scale
        )
    ...
    def rsample(self, shape):
        ...
        p = torch.distributions.Uniform(
            torch.zeros_like(self.mu_bar), torch.ones_like(self.sigma_bar)
        ).sample(shape)
...
class CensoredNormalDistribution(Distribution):
    ...
    def mean(self):
        ...
        E_z = TruncatedNormalDistribution(
            self.mu_bar, self.sigma_bar, self.a, self.b
        ).mean
        return (
    ...
    def variance(self):
        ...
        tn = TruncatedNormalDistribution(
            mu_bar=self.mu_bar, sigma_bar=self.sigma_bar, a=self.a, b=self.b
        )
        ...
        E_y2 = (
            self.b**2 * (1 - sn.cdf(beta))
            + self.a**2 * sn.cdf(alpha)
            + E_z2 * (sn.cdf(beta) - sn.cdf(alpha))
        )  # E(Y^2)
