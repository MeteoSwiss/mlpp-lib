...
class ValidDataSplitterOptions:
    ...
    def __post_init__(self):
        ...
        if self.time == "fractions":
            ...
            self.time_split = self.time_split_lists(self.reftimes)
            self.time_split_method = None
        elif self.time == "slices":
            self.time_split = self.time_split_slices(self.reftimes)
            self.time_split_method = None
        ...
        if self.station == "fractions":
            ...
            self.station_split = self.station_split_lists(self.stations)
            self.station_split_method = None
        elif self.station == "mixed":
            self.station_split = {"train": 0.7, "val": 0.3, "test": self.stations[-5:]}
            self.station_split_method = "random"

    def time_split_lists(self, reftimes):
        frac = {"train": 0.6, "val": 0.2, "test": 0.2}
        return ms.sequential_split(reftimes, frac)

    def time_split_slices(self, reftimes):
        out = {
            "train": [reftimes[0], reftimes[30]],
            "val": [reftimes[30], reftimes[40]],
            "test": [reftimes[40], reftimes[50]],
        }
        return out 

    def station_split_lists(self, stations):
        frac = {"train": 0.6, "val": 0.2, "test": 0.2}
        return ms.random_split(stations, frac)
...
class TestDataSplitter:
    ...
    @pytest.mark.parametrize("options", scenarios, ids=ValidDataSplitterOptions.pytest_id)
    def test_get_partition(self, options, features_dataset, targets_dataset):
        splitter = ms.DataSplitter(options.time_split, options.station_split, options.time_split_method, options.station_split_method)
        train_features, train_targets = splitter.get_partition(features_dataset, targets_dataset, partition="train")
        val_features, val_targets = splitter.get_partition(features_dataset, targets_dataset, partition="val")
        test_features, test_targets = splitter.get_partition(features_dataset, targets_dataset, partition="test")
    ...
    def test_time_split_method_required(self):
        ...
        with pytest.raises(ValueError) as excinfo:
            splitter = ms.DataSplitter(time_split, station_split, station_split_method="random")
        assert str(excinfo.value) == "`time_split_method` must be provided if the time splits are provided as fractions!"

    def test_station_split_method_required(self):
        time_split = {"train": 0.6, "val": 0.2, "test": 0.2}
        station_split = {"train": 0.6, "val": 0.2, "test": 0.2}
        with pytest.raises(ValueError) as excinfo:
            splitter = ms.DataSplitter(time_split, station_split, time_split_method="sequential")
        assert str(excinfo.value) == "`station_split_method` must be provided if the station splits are provided as fractions!"
    ...
    def test_station_split_keys_invalid(self):
        ...
        with pytest.raises(ValueError) as excinfo:
            splitter = ms.DataSplitter(time_split, station_split, time_split_method="sequential", station_split_method="random")
        assert str(excinfo.value) == "Time split and station split must be defined with the same partitions!"
    ...
    def test_time_split_method_invalid(self):
        ...
        with pytest.raises(ValueError) as excinfo:
            splitter = ms.DataSplitter(time_split, station_split, time_split_method=time_split_method, station_split_method=station_split_method)
        assert str(excinfo.value) == f"Invalid time split method: {time_split_method}. Must be one of {ms.DataSplitter.time_split_methods}."
    ...
    def test_station_split_method_invalid(self):
        ...
        with pytest.raises(ValueError) as excinfo:
            splitter = ms.DataSplitter(time_split, station_split, time_split_method=time_split_method, station_split_method=station_split_method)
        assert str(excinfo.value) == f"Invalid station split method: {station_split_method}. Must be one of {ms.DataSplitter.station_split_methods}."
